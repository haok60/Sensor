...
//Node mang su dung ham SenOneBye
void SendOneByte(BYTE data, WORD ClusterID);
//te de gui ban tin toi Router-EMB
//15/11/2016:
                ...
                case WSAN_Endpoint:
                {
                    switch (params.APSDE_DATA_indication.ClusterId.Val) {
                        ...
                        {//15/11/2016:
                            ...
                            break;
                        //TUNG
                        case STATUSTEMP_REQUEST:
                        {
                            APLGet();
                            SendOneByte(temp_req(),STATUSTEMP_RESPONSE);
                        }
                            break;
...
/*void Send_IMAGE_ToRouterEmboard(WORD CMR_Cluster) {
    WORD j;
    //DelayMs(100);
    TxBuffer[TxData++] = MAC_LONG_ADDR_BYTE0; //1byte
    TxBuffer[TxData++] = (BYTE) (length_data >> 8); //1byte
    TxBuffer[TxData++] = (BYTE) (length_data); //1byte
